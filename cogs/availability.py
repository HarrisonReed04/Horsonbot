...
from database import emojis, alphabet, availability_messages, work_schedule
...
class availability(commands.Cog):
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if str(payload.emoji) == str(emojis['clock']):
            ...
            try:
                ...
                await msg.edit(embed=discord.Embed(title=f"Great, I've noted that you are free FROM {usrinput.content}.", color=0xf0fff0))
        ...
        if str(payload.emoji) == str(emojis['alarmclock']):
            ...
            try:
                usrinput = await self.bot.wait_for('message', check=check, timeout=60)
                await msg.edit(embed=discord.Embed(title=f"Great, I've noted that you are free UNTIL {usrinput.content}.", color=0xf0fff0))
        ...
        if find(embed.to_dict()['fields'],"name",f"{user}") == None:
            embed.add_field(name =f"Available" if str(payload.emoji) == emojis['greentick'] else "Unsure" if str(payload.emoji) == emojis['?'] else f"Available from `{usrinput.content}`" if str(payload.emoji) == emojis['clock'] else "Unavailable" if str(payload.emoji) == emojis['redtick'] else f"Available until `{usrinput.content}`" if str(payload.emoji) == emojis['alarmclock'] else "Somehow you've messed up properly",inline=False)    
        else:
        ...
        if "Available from" in embedx['fields'][index]['value'] and str(payload.emoji) == emojis['alarmclock']:
            if " until " not in embedx['fields'][index]['value']:
                await self.edit_embed(message.id, channel, index,name=name,value=f"{embedx['fields'][index]['value']} until `{usrinput.content}`",inline=False)
            else:
                await self.edit_embed(message.id, channel, index,name=name,value=f"{(' '.join((embedx['fields'][index]['value'].split(' '))[:-2]))} until `{usrinput.content}`",inline=False)      
            ...
            if "Available from" not in embed['fields'][index]['value']:
                await self.edit_embed(message.id, channel, index,name=name,value=f"Available from `{usrinput.content}` {embedx['fields'][index]['value'][10:]}",inline=False)
            else:
                await self.edit_embed(message.id, channel, index,name=name,value=f"Available from `{usrinput.content}` {' '.join((embedx['fields'][index]['value'].split(' '))[3:])}",inline=False)
        else:
            await self.edit_embed(msgid=message.id, channel=channel, index=index,name=name,value=f"Available" if str(payload.emoji) == emojis['greentick'] else "Unsure" if str(payload.emoji) == emojis['?'] else f"Available from `{usrinput.content}`" if str(payload.emoji) == emojis['clock'] else "Unavailable" if str(payload.emoji) == emojis['redtick'] else f"Available until `{usrinput.content}`" if str(payload.emoji) == emojis['alarmclock'] else "Somehow you've messed up properly",inline=False) 
        try:
    ...
    @commands.command()
    async def work(self, ctx, date:str=None):
        embed = discord.Embed(title="Harrison's Work Schedule!", color = 0x99ff99)
        days = []
        daysnames = []
        if not date:
            for x in range(0,6):
                days.append((datetime.now()+timedelta(days=x)).strftime("%d/%m/%Y"))
                daysnames.append((datetime.now()+timedelta(days=x)).strftime("%A"))
            title = f"{emojis['alarmclock']}{daysnames[0]} {days[0]} - {daysnames[-1]} {days[-1]}{emojis['alarmclock`']}"
            text = f"\n"
            for y in range(0,len(days)):
                text += f"**{daysnames[y]} {days[y]}**⠀⠀⠀⠀{emojis['clock']}{work_schedule[days[y]]}\n"
